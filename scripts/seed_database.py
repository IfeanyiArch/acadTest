#!/usr/bin/env python
"""
Database seeding script for Acad AI Assessment Engine

Usage:
    python scripts/seed_database.py              # Seed with default data
    python scripts/seed_database.py --test       # Seed with test data
    python scripts/seed_database.py --flush      # Flush database first
"""
...
class DatabaseSeeder:
    ...
    def create_exams_with_questions(self, courses, exams_per_course=2):
        ...
        print(f"📋 Creating exams with questions...")
    ...
    def create_submissions(self, users, exams, submissions_per_user=3):
        ...
        print(f"✍️  Creating submissions...")
        ...
        for user in users[:5]:  # Only first 5 users submit
            for exam in exams[:submissions_per_user]:
                ...
                if len(submissions) % 2 == 0:
                    from apps.assessment.services.grading.factory import (
                        get_grading_service,
                    )
